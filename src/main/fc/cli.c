...
static void printGeozones(uint8_t dumpMask, const geoZoneConfig_t *geoZone, const geoZoneConfig_t *defaultGeoZone)
{
    ...
    for (uint8_t i = 0; i < MAX_GEOZONES_IN_CONFIG; i++) {
        bool equalsDefault = false;
        if (defaultGeoZone) {
            equalsDefault = geoZone[i].fenceAction == defaultGeoZone[i].fenceAction
            && geoZone[i].shape == defaultGeoZone[i].shape
            && geoZone[i].type == defaultGeoZone[i].type
            && geoZone[i].maxAltitude == defaultGeoZone[i].maxAltitude
            && geoZone[i].maxAltitude == defaultGeoZone[i].minAltitude;
...
{  
    if (isEmpty(cmdLine)) {
        ...
        if ((ptr = nextArg(ptr))){
            ...
            if (fenceAction < 0 || fenceAction > FENCE_ACTION_RTH) {
                cliShowArgumentRangeError("fence action", 0, FENCE_ACTION_RTH);
                return;
            }
        } else {
            cliShowParseError();
...
static void cliStatus(char *cmdline)
{
    ...
    while (flags) {
        ...
        flags &= ~(1 << bitpos);
	if (bitpos > 5) cliPrintf(" %s", armingDisableFlagNames[bitpos - 6]);
    }
    cliPrintLinefeed();
...
static void printConfig(const char *cmdline, bool doDiff)
{
    ...
    if ((dumpMask & DUMP_MASTER) || (dumpMask & DUMP_ALL)) {
        ...
        cliPrintHashLine("geozonee vertices");
