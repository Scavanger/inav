...
void calcCenterOfPoints(fpVector2_t *result, const fpVector2_t* vertices, int vertexCount)
{
	fpVector2_t sum = {.x = 0, .y = 0};
	for (int i = 0; i < vertexCount; ++i) {
		vector2Add(&sum, &sum, &vertices[i]);
	}
	vector2Scale(result, &sum, 1.0f / vertexCount);	
}
