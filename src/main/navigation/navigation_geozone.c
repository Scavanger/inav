...
#define STICK_LOCK_MIN_TIME 5000
...
static uint32_t getDetectionDistance(void)
{
...
static int8_t calcRthCourse(fpVector3_t* waypoints, const fpVector3_t* point, fpVector3_t* target)
{
    ...
    float offset = getDetectionDistance();
    ...
    for (uint8_t i = 0 ; i < activeGeoZonesCount; i++) {
        ...
        /*
        if (activeGeoZones[i].config.maxAltitude + geoZoneConfig()->safeAltitudeDistance < start->z) {
            continue;
        } 
        */       
        ...
        offset = getDetectionDistance() * 2 / 3;
...
void performeFenceAction(geoZoneRuntimeConfig_t *zone, fpVector3_t *intersection)
{
    ...
    if (zone->config.fenceAction == FENCE_ACTION_AVOID) {     
        ...
        if ((zone->config.type == GEOZONE_TYPE_EXCLUSIVE && geozone.zoneInfo > 0 && (geozone.zoneInfo < geoZoneConfig()->avoidAltitudeRange)) || avoidFzStep)  {
            ...
            // Check for min/Mmax altitude

            if (geozone.currentzoneMaxAltitude > 0 && getEstimatedActualPosition(Z) > geozone.currentzoneMaxAltitude) {
                posControl.sendTo.targetPos.z = geozone.currentzoneMaxAltitude - geoZoneConfig()->safeAltitudeDistance * 2;
            } else if (geozone.currentzoneMinAltitude != 0 && getEstimatedActualPosition(Z) < geozone.currentzoneMinAltitude) {
                posControl.sendTo.targetPos.z = geozone.currentzoneMinAltitude + geoZoneConfig()->safeAltitudeDistance * 2;
            }
    ...
    } else if (zone->config.fenceAction == FENCE_ACTION_POS_HOLD) {
        ...
        actionStartTime = millis();
    } else if (zone->config.fenceAction == FENCE_ACTION_RTH) {
...
static void geoZoneInit(void)
{
    ...
    for (uint8_t i = 0; i < MAX_GEOZONES_IN_CONFIG; i++)
    {
        if (geoZonesConfig(i)->vertexCount > 0) {
            ...
            if (activeGeoZones[i].config.maxAltitude == INT32_MAX && activeGeoZones[i].config.minAltitude == 0) {
                 activeGeoZones[i].isInfZone = true;
            } else {
                activeGeoZones[i].isInfZone = false;
            }
            

            if (!STATE(FIXED_WING_LEGACY) && activeGeoZones[i].config.fenceAction == FENCE_ACTION_AVOID){
                activeGeoZones[i].config.fenceAction = FENCE_ACTION_POS_HOLD;
            }
...
{   
    ...
    // Zome RTH Override: Toggle RTH switch short
    ...
    /*
    if (getEstimatedActualPosition(Z) < geoZoneConfig()->safeAltitudeDistance) {
        return;
    }
    */

    // User has switched to non gefence mode, end all actions and switch to mode from box input
    ...
    {
        case GEOZONE_ACTION_STATE_AVOIDING: 
            if (calculateDistanceToDestination(&avoidingPoint) > getDetectionDistance()) {
                posControl.sendTo.lockSticks = false;
            }            
    ...
    for (uint8_t i = 0; i < currentZoneCount; i++) {
        if (currentZones[i]->config.fenceAction != FENCE_ACTION_NONE) {
    ...
    if ((IS_RC_MODE_ACTIVE(BOXHORIZON) || IS_RC_MODE_ACTIVE(BOXANGLE)) && currentZoneHasAction && (geozone.currentzoneMaxAltitude > 0 || geozone.currentzoneMinAltitude != 0) && ABS(geozone.distanceVertToNearestZone ) < geoZoneConfig()->safeAltitudeDistance) {
        ...
        if (!geozone.insideFz) {
            if (geozone.distanceVertToNearestZone < 0) {
                targetAltitide = nearestInclusiveZone->config.maxAltitude - geoZoneConfig()->safeAltitudeDistance * 2;
            } else {
                targetAltitide = nearestInclusiveZone->config.minAltitude + geoZoneConfig()->safeAltitudeDistance * 2;
            }
        } else {
            targetAltitide = geozone.distanceVertToNearestZone > 0 ? geozone.currentzoneMaxAltitude - geoZoneConfig()->safeAltitudeDistance * 2 : geozone.currentzoneMinAltitude + geoZoneConfig()->safeAltitudeDistance * 2;
        }
    ...
    if ((navGetCurrentStateFlags() & NAV_AUTO_RTH) || IS_RC_MODE_ACTIVE(BOXNAVRTH) || posControl.flags.forcedRTHActivated) {
        return;
    } else {
        if (geozone.avoidInRTHInProgress) {
            geozoneResetRTH();
    ...
    if (geozone.insideNfz || (!geozone.insideFz && isAtLeastOneInclusiveZoneActive)) {
        ...
        if (!isNonGeozoneModeFromBoxInput()) {
            ...
            for (uint8_t i = 0; i < currentZoneCount; i++) {
                if (currentZones[i]->config.type == GEOZONE_TYPE_EXCLUSIVE && currentZones[i]->config.fenceAction != FENCE_ACTION_NONE) {
                    flyOutNfz = true;
            ...
            if (targetZone != NULL && !lockRTZ && (flyOutNfz || (!geozone.insideFz && targetZone->config.fenceAction != FENCE_ACTION_NONE))) {
