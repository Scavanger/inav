...
{   
    ...
    if (!ARMING_FLAG(ARMED) || !isGPSHeadingValid() || !isInitalised || activeGeoZonesCount == 0 || FLIGHT_MODE(NAV_FW_AUTOLAND) || ((STATE(AIRPLANE) && (navGetCurrentStateFlags() & NAV_CTL_LAUNCH))) ) {
        noZoneRTH = false;
        return;
    } 
